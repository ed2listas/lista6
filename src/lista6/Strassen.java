...
public class Strassen
{
    ...
    public void start (JFrame screen) 
    {
        ...
        for (int i = 0; i < 2; i++) {
        	matrixA += "| ";
            for (int j = 0; j < 2; j++) {
            	A[i][j] = numbers.get(generate_random_numbers.nextInt(1600));
        ...
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++)
            	B[i][j] = numbers.get(generate_random_numbers.nextInt(1600));
        ...
        JLabel Titulo = new JLabel(matrixA);
        matrix.add(Titulo);
        matrix.setBounds(100,200,1600,800);
        screen.add(matrix);
